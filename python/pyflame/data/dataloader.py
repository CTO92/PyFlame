...
class DataLoaderIterator:
    ...
    Note: This iterator is designed for map-style datasets only.
    For iterable datasets, use DataLoader._iter_iterable_dataset() directly.
    """
    ...
    def __init__(self, loader: DataLoader):
        ...
        self._num_yielded = 0
        # Cache length to avoid repeated len() calls that may fail
        try:
            self._total_batches = len(loader)
        except TypeError:
            self._total_batches = None  # IterableDataset doesn't support len()

    def __iter__(self):
        return self

    def __next__(self):
        # For map-style datasets, check if we've yielded all batches
        if self._total_batches is not None and self._num_yielded >= self._total_batches:
            raise StopIteration
    ...
    def __len__(self):
        if self._total_batches is None:
            raise TypeError(
                "DataLoaderIterator length is not defined for IterableDataset"
            )
